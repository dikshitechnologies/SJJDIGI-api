...
(
    ...
    LEFT JOIN PARTY PARENT
        ON PARENT.FPARENT =
           LEFT(P.FPARENT, LEN(P.FPARENT) - 5)
...
ORDER BY FACNAME;";
                ...
                command.Parameters.Add(
                    ...
                    SqlDbType.VarChar,
                    50
                ).Value = string.IsNullOrWhiteSpace(schemeCode)
                    ? DBNull.Value
                    : schemeCode;
                ...
                while (await reader.ReadAsync())
                {
                    list.Add(new
                    {
                        FCode = reader["FCODE"] == DBNull.Value
                            ? null
                            : reader["FCODE"].ToString(),

                        FACNAME = reader["FACNAME"] == DBNull.Value
                            ? null
                            : reader["FACNAME"].ToString(),

                        FPHONE = reader["FPHONE"] == DBNull.Value
                            ? null
                            : reader["FPHONE"].ToString(),

                        FAMOUNT = reader["FAMOUNT"] == DBNull.Value
                            ? 0
                            : Convert.ToDecimal(reader["FAMOUNT"]),

                        FCOMPCODE = reader["FCOMPCODE"] == DBNull.Value
                            ? null
                            : reader["FCOMPCODE"].ToString(),
                        ...
                        MaturityDate = reader["MaturityDate"] == DBNull.Value
                            ? null
                            : Convert.ToDateTime(reader["MaturityDate"])
                                .ToString("yyyy-MM-dd"),

                        FDUE = reader["FDUE"] == DBNull.Value
                            ? 0
                            : Convert.ToInt32(reader["FDUE"]),

                        SCHEMECODE = reader["SCHEMECODE"] == DBNull.Value
                            ? null
                            : reader["SCHEMECODE"].ToString(),

                        PaidDue = reader["PaidDue"] == DBNull.Value
                            ? 0
                            : Convert.ToInt32(reader["PaidDue"]),
                        ...
                        TotalBenefitAmt = reader["TotalBenefitAmt"] == DBNull.Value
                            ? "0.00"
                            : Convert.ToDecimal(reader["TotalBenefitAmt"])
                                .ToString("0.00"),

                        TotalBenefitWt = reader["TotalBenefitWt"] == DBNull.Value
                            ? "0.000"
                            : Convert.ToDecimal(reader["TotalBenefitWt"])
                                .ToString("0.000"),

                        SCHEMENAME = reader["SCHEMENAME"] == DBNull.Value
                            ? null
                            : reader["SCHEMENAME"].ToString(),

                        ActiveStatus = reader["ActiveStatus"] == DBNull.Value
                            ? null
                            : reader["ActiveStatus"].ToString(),

                        // NEW FIELD
                        FDIGITYPE = reader["FDIGITYPE"] == DBNull.Value
                            ? null
                            : reader["FDIGITYPE"].ToString()
                    });
                }
